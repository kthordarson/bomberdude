...
def drawbox(sx, ex, sy, ey, color, lw):
    ...
    fontsize = 10
    draw_line(
        start_x=sx, start_y=sy, end_x=sx + textwidth, end_y=sy, color=color
    )  # bottom line
    draw_line(
        start_x=sx,
        start_y=sy + fontsize,
    ...
    draw_line(
        start_x=sx, start_y=sy, end_x=sx, end_y=sy + fontsize, color=color
    )  # left line
    draw_line(
        start_x=sx + textwidth,
        start_y=sy,
        end_x=sx + textwidth,
        end_y=sy + fontsize,
        color=color,
...
def draw_debug_players(players, labels, camera):
    ...
    for idx, p in enumerate(players):
        ...
        draw_circle_filled(
            center_x=fp.x, center_y=fp.y, radius=7, color=arcade.color.BLACK
        )
    ...
    if len(poslist) > 0:
        ...
        for pl in poslist:
            ...
            draw_circle_filled(
                center_x=fpx, center_y=fpy, radius=4, color=arcade.color.GREEN
            )
            draw_line(
                start_x=fpx,
                start_y=fpy,
                end_x=camera.viewport_width // 2,
                end_y=camera.viewport_height // 2,
...
def render_widget_debug(idx, item, text_pos, fontsize):
    ...
    draw_circle_filled(
        center_x=item.x, center_y=item.y, radius=2, color=arcade.color.ORANGE
    )  # .
    draw_circle_filled(
        center_x=item.x + item.width,
        center_y=item.y,
        radius=2,
