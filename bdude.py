...
class Game(Thread):
    def __init__(self, args=None):
        ...
        self.display = pygame.display.set_mode(
            (800, 800)
        )  # pygame.display.set_mode(size=(800,800), flags=pygame.DOUBLEBUF, vsync=1).get_surface()
        ...
        self.player = NewPlayer(
            image=self.rh.get_image("data/playerone.png"),
            rh=self.rh,
            serveraddress=(self.args.server, self.args.port),
        )
        self.bombs = Group()
        self.flames = Group()
    ...
    def player_collide(self):
        for f in self.flames:  # check flame collisions with player
            if collide_rect(f, self.player):
                ...
                self.player.health -= (
                    f.damage
                )  # todo fix this, server should keep track of this
                if self.player.health <= 0:  # dead
                    self.player.playerlist[f.bomberid]["score"] += (
                        1  # increase score of bomber
                    )
                    logger.warning(
                        f"playerkilled {self.player.client_id} by f: {f.bomberid} "
                    )  # pl:{self.player.playerlist[f.bomberid]}
                    ...
                    logger.info(
                        f"playerflamedamage f: {f.bomberid} ph: {self.player.health}"
                    )
            ...
            for c in colls:
                ...
                match c.blocktype:
                    ...
                    case 3:  # killable block, no upgrade, turns into backgroundblock
                        ...
                        image = self.rh.get_image("data/blocksprite2.png")
                        self.blocks.add(
                            NewBlock(gridpos=fgpos, image=image, blocktype=2)
                        )
                        payload = {
                            "msgtype": "cl_gridupdate",
                            "gridpos": c.gridpos,
                            "blocktype": 2,
                            "client_id": self.player.client_id,
                            "grid": self.player.grid,
                        }
                    ...
                    case 4:  # solid killable creates upgradeblock type 40/44
                        ...
                        if upgrade_type == 40:
                            image = self.rh.get_image("data/newbomb.png")
                        if upgrade_type == 44:
                            image = self.rh.get_image("data/heart.png")
                        # image = self.rh.get_image(f'data/blocksprite2.png')
                        self.upgradeblocks.add(
                            UpgradeBlock(
                                gridpos=fgpos, image=image, blocktype=upgrade_type
                            )
                        )  # type 44 = heart
                        payload = {
                            "msgtype": "cl_gridupdate",
                            "gridpos": c.gridpos,
                            "blocktype": upgrade_type,
                            "client_id": self.player.client_id,
                            "grid": self.player.grid,
                        }
                        self.player.send_queue.put(
                            payload
                        )  # tell server and kill block and flame
                        c.kill()
                        f.kill()
    ...
    def update_screen(self):
        ...
        for row in self.player.grid:
            ...
            for k in row:
                ...
                image = self.rh.get_image(f"data/blocksprite{k}.png")
                self.blocks.add(
                    NewBlock(gridpos=(y, x), image=image, blocktype=k)
                )  # swap x,y for gridpos
                x += 1  # BLOCK
            y += 1  # BLOCK
        # self.display.blit(newsurf, (0,0))
    ...
    def draw_debug_info(self):
        ...
        dbgitems = []
        dbgitems.append(
            self.debugfont.render(
                f"fps: {self.clock.get_fps():.2f} blks:{len(self.blocks)} b:{len(self.bombs)} f:{len(self.flames)} p:{len(self.particles)} ub:{len(self.upgradeblocks)}  ",
                (255, 255, 255),
            )
        )
        dbgitems.append(
            self.debugfont.render(
                f"pc: {self.player.updcntr} rc: {self.player.runcounter} s: {self.player.sendcounter} r: {self.player.recvcounter} rq: {self.player.receiverq.qsize()} sq: {self.player.send_queue.qsize()} mq: {self.player.msg_queue.qsize()}",
                (255, 255, 255),
            )
        )
        dbgitems.append(
            self.debugfont.render(
                f"threads: {active_count()} server: {self.server_running}  ",
                (255, 255, 255),
            )
        )
        if len(self.player.debuginfo["server"]) > 0:
            dbgitems.append(
                self.debugfont.render(
                    f"sv_updcntr: {self.player.debuginfo['server'].get('sv_updcntr')}",
                    (255, 255, 255),
                )
            )
            for k in self.player.debuginfo.get("server").get("clients"):
                dbgitems.append(
                    self.debugfont.render(
                        f"id: {k.get('client_id')} handlerq: {k.get('handlerq')}",
                        (255, 255, 255),
                    )
                )
        for d in dbgitems:
    ...
    def draw_game_info(self):
        surf, rect1 = self.font.render(
            f"h: {self.player.health} bombs: {self.player.bombsleft} score: {self.player.score}",
            (255, 255, 255),
        )
        self.display.blit(surf, (10, 10))
        surf, rect = self.font.render(
            f"{self.player.health} ", (5, 255, 55)
        )  # {self.player.playerlist[self.player.client_id]["health"]}
        self.display.blit(surf, (self.player.pos))
        for np in self.player.playerlist:
            if np != self.player.client_id:
                surf, rect = self.font.render(
                    f"h: {self.player.playerlist[np]['health']} bombs: {self.player.playerlist[np]['bombsleft']} score: {self.player.playerlist[np]['score']}",
                    (155, 255, 255),
                )
                self.display.blit(surf, (rect1.width + 30, 10))
                surf, rect = self.font.render(
                    f"{self.player.playerlist[np]['health']} ", (255, 5, 55)
                )
    ...
    def server_thread(self):  # todo threading.lock for this
        ...
        while True:
            ...
            conncounter += 1
            logger.info(
                f"{self} {server} NewHandler started {thread} conn: {conncounter} clients: {len(server.clients)}"
            )
    ...
    def handle_input_events(self, event):
        ...
        if keypressed in (pygame.K_q, 113, "q", "Q"):
            ...
            if not self.show_mainmenu:
                ...
                if self.game_menu.active_item == "Start":
                    if not self.game_started:
                        ...
                        logger.warning("game already started")  # todo handle this
                        ev = Event(
                            USEREVENT,
                            payload={
                                "msgtype": "startgame",
            ...
            logger.debug(
                f"K_ESCAPE item: {self.game_menu.active_item} show: {self.show_mainmenu}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--testclient", default=False, action="store_true", dest="testclient"
    )
