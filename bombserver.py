...
import threading
from threading import Thread, active_count, _enumerate
...
class ServerTUI(Thread):
    ...
    def dump_playerlist(self):
        ...
        for p in self.server.playerlist:
            print(
                f"\tclid: {p} pos: {self.server.playerlist[p].get('pos')} {self.server.playerlist[p].get('gridpos')} b: {self.server.playerlist[p].get('bombsleft')} h: {self.server.playerlist[p].get('health')} uc: {self.server.playerlist[p].get('updcntr')} "
            )
        ...
        while True:
            ...
            try:
                ...
                if cmd[:1] == "p":
                    ...
                    logger.info("[S] cmds: s serverinfo, p playerlist, q quit")
...
class NewHandler(Thread):
    def __init__(self, conn, addr, handlerq, name):
    ...
    def run(self):
        while self.connected:
            ...
            try:
                ...
                logger.error(
                    f"{self.client_id} disconnected {e} {type(e)}"
            ...
            try:
                ...
                logger.error(
                    f"{self} {e} {type(e)} m:{msglen} rawmsglen: {type(rawmsglen)}\raw: {rawmsglen}"
                )
                self.connected = False
                self.conn.close()
                break
            ...
            if self.handlerq.qsize() > 5:
                logger.warning(f"{self} handlerq: {self.handlerq.qsize()}")
            ...
            if not cl.client_id:
                ...
                try:
                    if not cl.connected:
                        ...
                        logger.warning(
                            f"{self} {cl.client_id} disconnected playerlist: {len(self.playerlist)}"
                        )
    ...
    def msg_handler(self, data):
        ...
        match msgtype:
            case "cl_serverdebug":  # client request debuginfo
                ...
                for client in self.clients:
                    msg["dbginfo"]["clients"].append(
                        {
                            "client_id": client.client_id,
                            "name": client.name,
                            "handlerq": client.handlerq.qsize(),
                        }
                    )
                for client in self.clients:
                    try:
            ...
            case "cl_playerbomb":  # player sent bomb, update others
                ...
                logger.info(
                    f"{msgtype} dclid: {data.get('client_id')} clbombpos: {data.get('clbombpos')}"
                )
                for client in self.clients:
            ...
            case (
                ...
                logger.info(
                    f"{msgtype} dclid: {data.get('client_id')} "
                )  # playerlist: {self.playerlist}
                for client in self.clients:
                    msg = {
                        "msgtype": "sv_playerlist",
                        "playerlist": self.playerlist,
                    }
                    try:
                        self.do_send(client.conn, client.addr, msg)
            ...
            case _:
                logger.warning(
                    f"unknown msgtype {msgtype} from {data.get('client_id')}\ndata: {data}"
                )
