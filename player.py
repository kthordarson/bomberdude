...
class NewPlayer(Thread):
	...
	def run(self):
		if self.connect():
			self.sender.start()
			self.receiver.start()
			payload = {'msgtype' : 'cl_newplayer', 'client_id': self.client_id, 'c_pktid': gen_randid()}
			# data = json.dumps(payload).encode('utf-8')
			send_data(conn=self.socket, payload=payload, pktid=gen_randid())
			logger.debug(f'{self} connectrun sent: {len(payload)} {type(payload)} {payload}')
		else:
		...
		while not self.kill:
			...
			while not self.receiver.queue.empty():
				...
				logger.debug(f'payload  {len(payload)} {type(payload)}\n{payload}')
				if payload:
	...
	def handle_payloadq(self, rawpayload):
		...
		if msgtype == 'bcsetclid':
			...
			logger.debug(f'{self} bcsetclid payload={payload}')
	...
	def oldhandle_payloadq(self, payloads):
		...
		for idx,payload in enumerate(payloads):
			logger.debug(f'{idx}/{len(payloads)} payload {len(payload)} {type(payload)}\n{payload}')
			if len(payload) == 0:
			...
			if isinstance(payload, str):
				# logger.warning(f'{idx}/{len(payloads)} payload {len(payload)} {type(payload)} : {payload}')
				# logger.warning(f'payloads {len(payloads)} {type(payloads)}:\n\n{payloads}\n\n')
				payload = json.loads(json.dumps(payloads))
			try:
				msgtype = payload.get('msgtype')
				in_pktid = payload.get('pktid')
			except AttributeError as e:
				logger.error(f'{idx}/{len(payloads)}  {e} payload: {type(payload)} {payload}')
				logger.error(f'payloads {type(payloads)} {len(payloads)} :\n{payloads}\n')
				break
			if msgtype == 'bcsetclid':
				self.client_id = payload.get('client_id')
				self.sender.client_id = self.client_id
				self.receiver.client_id = self.client_id
				logger.debug(f'{idx}/{len(payloads)} {self} bcsetclid payload={payload}')
				# clid = payload.get('client_id')
				# self.set_clientid(clid)
			if msgtype == 's_netplayers':
				# logger.debug(f'netplayers payload={payload}')
	...
	def oldconnect(self):
		...
		while not self.connected:
			...
			if attemtps >= 10:
				...
				return False
			payload = {'msgtype' : 'oldcl_newplayer', 'client_id': self.client_id, 'c_pktid': gen_randid()}
			# data = json.dumps(payload).encode('utf-8')
			send_data(conn=self.socket, payload=payload, pktid=gen_randid())
			rawdata_sock = None
			logger.debug(f'attemtps: {attemtps} sent: {len(payload)} {type(payload)} {payload}')
			try:
			...
			if rawdata_sock:
				...
				if data.get('msgtype') == 's_ping':
					logger.debug(f'connect s_ping: {rawdata_sock}')
				elif data.get('msgtype') == 'oldbcsetclid':
					logger.debug(f'oldbcsetclid: data: {data} raw: {rawdata_sock}')
					self.client_id = data.get('client_id')
					self.sender.start()
					self.receiver.start()
					self.connected = True
					logger.debug(f'oldbcsetclid connected s:{self.sender} r:{self.receiver} rawdata: {rawdata_sock}')
					return True
