...
from arcade.gui import UIManager, UIGridLayout
...
from objects import Bomberplayer, Bomb, BiggerBomb, KeysPressed, PlayerEvent, UIPlayerLabel, Bullet
...
class Bomberdude(arcade.View):
	...
	def respawn_playerone(self):
		...
		self.game_eventq.put_nowait(repawnevent)
	...
	def do_connect(self):
		...
		connection_event = {
			...
			'eventid': gen_randid()}
		self.game_eventq.put_nowait(connection_event)
		self.setup_network()
	...
	def on_draw(self):
		...
		if self.debugmode and BULLETDEBUG:
			for b in self.game_state.scene['Bullets']:
				if b.can_kill:
					self.camera.use()
					draw_line(start_x=b.center_x, start_y=b.center_y, end_x=self.playerone.center_x,
					          end_y=self.playerone.center_y, color=arcade.color.ORANGE, line_width=1)
					textpos = get_map_coordinates_rev(b.position, self.camera)
					self.guicamera.use()
					try:
						textpos += Vec2d(10, 0)
						arcade.Text(text=f'bxc: {b.change_x:.2f} bcy: {b.change_y:.2f} ', start_x=int(textpos.x),
						            start_y=int(textpos.y), color=arcade.color.BLACK, font_size=10).draw()
						textpos += Vec2d(0, 11)
						arcade.Text(text=f'ba: {b.angle:.2f}', start_x=int(textpos.x), start_y=int(textpos.y),
						            color=arcade.color.BLACK, font_size=10).draw()
	...
	def on_mouse_press(self, x, y, button, modifiers):
		...
		if button == 1:
			...
			bulletfiredevent = {
				...
				'eventid': gen_randid()}
			self.game_eventq.put_nowait(bulletfiredevent)
		else:
	...
	def on_key_press(self, key, modifiers):
		...
		if key == arcade.key.KEY_1:
			...
			self.debugtrace = not self.debugtrace
			logger.debug(f'self.debugtrace: {self.debugtrace}')
			...
			self._connected = False
			quitevent = {'event_time': 0, 'event_type': 'playerquit', 'handled': True, 'handledby': self.playerone.client_id, 'client_id': self.playerone.client_id, 'eventid': gen_randid()}
			self.game_eventq.put_nowait(quitevent)
	...
	def on_key_release(self, key, modifiers):
		...
		if key == arcade.key.UP or key == arcade.key.DOWN or key == arcade.key.W or key == arcade.key.S:
			...
			self.playerone.dropbomb(self.selected_bomb, self.game_eventq)
	...
	def handle_game_events(self, game_event):
		...
		match event_type:
			case 'dupeevent':
				logger.warning(f'{event_type=} {game_event=}')
			case 'extrabomb':
			...
			case 'ackrespawn':
				[self.netplayers[k].set_texture(arcade.load_texture('data/netplayer.png')) for k in self.netplayers
					if k == clid]  # [0]
			...
			case 'playerkilled' | 'dmgkill':
				...
				[self.netplayers[k].set_texture(arcade.load_texture('data/netplayerdead.png')) for k in
					self.netplayers if k == dmgto]  # [0]
		...
		if self.playerone.client_id in self.game_state.players:
			try:
				...
				try:
					...
					logger.error(f'{e} {playeronedata=}')
			except KeyError as e:
		...
		for gsplr in gsplr_copy:  #
			...
			# _ = [self.netplayers[k].set_data(self.game_state.players[pclid]) for k in self.netplayers if k == pclid] #  and k != self.playerone.client_id
				...
				if pclid == self.playerone.client_id:
					...
					newplayer = Bomberplayer(texture="data/playerone.png", client_id=pclid, name=name,
				...
				# playerlabel.button.text = f'Me {name}'
					...
					newplayer = Bomberplayer(texture="data/netplayer.png", client_id=pclid, name=name,
					                         position=position_fix)
				...
				playerlabel = UIPlayerLabel(client_id=str(newplayer.client_id), name=name,
				                            text_color=arcade.color.GREEN)
				playerlabel.button.text = f'{name}'
				playerpanel = Panel(10, 101 - (len(self.netplayer_panels) * 12), 250, 50, owner=pclid,
				                    window=self.window)
				if pclid != self.playerone.client_id:
	...
	def on_update(self, dt):
		...
		for b in self.game_state.scene['Bullets']:
			try:
				...
				logger.error(f"{e=} {b=} bullets: {len(self.game_state.scene['Bullets'])}")
				traceback.print_exception(e)
				traceback.print_stack()
				break
			if arcade.check_for_collision(b, self.playerone):
				takedamageevent = {'event_time': 0, 'event_type': 'takedamage', 'dmgto': self.playerone.client_id, 'dmgfrom': b.shooter, 'damage': b.damage, 'handled': False, 'eventid': gen_randid()}
				if self.debugmode:
					logger.debug(f'takedamage {self.playerone} from {b.shooter} {b.damage=}')
				self.game_eventq.put_nowait(takedamageevent)
				try:
		...
		for f in self.game_state.scene['Flames']:
			...
			for hit in arcade.check_for_collision_with_list(f, self.game_state.scene['Blocks']):
				...
				match hitblocktype:
					case 2:
						blkxplodeevent = {'event_time': 0, 'event_type': 'blkxplode', 'hit': hitblocktype, 'flame': f.position, 'fbomber': f.bomber, 'client_id': self.playerone.client_id, 'handled': False, 'handledby': 'playerone', 'eventid': gen_randid()}
						self.game_eventq.put_nowait(blkxplodeevent)
						self.game_state.scene['Blocks'].remove(hit)
					case 3:
						hit.hit_count += 1
						logger.debug(f'hitcount: {hit.hit_count} {hit=}')
						if hit.hit_count > 3:
							blkxplodeevent = {'event_time': 0, 'event_type': 'blkxplode', 'hit': hitblocktype,
							         'flame': f.position, 'fbomber': f.bomber, 'client_id': self.playerone.client_id,
							         'handled': False, 'handledby': 'playerone', 'eventid': gen_randid()}
							self.game_eventq.put_nowait(blkxplodeevent)
							self.game_state.scene['Blocks'].remove(hit)
					case 6:
						blkxplodeevent = {'event_time': 0, 'event_type': 'blkxplode', 'hit': hitblocktype, 'flame': f.position,
						         'fbomber': f.bomber, 'client_id': self.playerone.client_id, 'handled': False,
						         'handledby': 'playerone', 'eventid': gen_randid()}
						self.game_eventq.put_nowait(blkxplodeevent)
						self.game_state.scene['Blocks'].remove(hit)
					case 9:
						blkxplodeevent = {'event_time': 0, 'event_type': 'blkxplode', 'hit': hitblocktype, 'flame': f.position,
		...
		for b in self.game_state.scene["Bombs"]:
			try:
				b.update(self.game_state.scene, self.game_eventq)
			except KeyError as e:
				logger.error(f'{e} {b=}')
		for upgr in self.game_state.scene['Upgrades']:
			...
			if arcade.check_for_collision(upgr, self.playerone):
				...
				takeupgradeevent = {'event_time': 0, 'event_type': 'takeupgrade', 'upgradetype': upgradetype, 'client_id': self.playerone.client_id, 'handled': False, 'eventid': gen_randid()}
				if self.debugmode:
					logger.debug(f'pickedup upgr: {upgr} {upgradetype=} {self.playerone} ')
				self.game_eventq.put_nowait(takeupgradeevent)
